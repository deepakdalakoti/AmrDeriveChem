      ...
      char* argv[])
{
	...
	std::cout << "X from " << idXin << " in pltfile to " << idXlocal << " local, to " << idCout << " in output" << endl;
    }
    ...
    for (int lev=0; lev<Nlev; ++lev)
    {
        ...
        {
            const FArrayBox& Xlocal = indata[mfi];
            ...
            Yt.resize(box, nSpec);
	    int idYtmp = 0;
	    cd.moleFracToMassFrac(Yt,Xlocal,box,idXlocal,idYtmp);
            cd.normalizeMassFrac(Yt,Yt,"N2",box,idYtmp,idYtmp);
