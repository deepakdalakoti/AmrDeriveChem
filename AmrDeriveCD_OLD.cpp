      ...
      char* argv[])
{
    ...
    for (int i=0; i<nSpec; ++i) {
        varNames[sCompY + i] = "Y(" + cd.speciesNames()[i] + ")";
    }
    ...
    {
        CD.set(iLevel,new MultiFab(amrData.boxArray(iLevel),2*cdNames.size(),0,Fab_allocate));
	...
	if (ParallelDescriptor::IOProcessor()) {
	  std::cout << "\nFAB byte spread across MPI nodes: ["
	    ...
	    cd.getHmixGivenTY(RhoH,T,RhoY,box,sCompT,sCompY,0);

	    RhoH.mult(Rho,sCompR,0,1);
	    for (int i=0; i<nSpec; ++i) {
	      RhoY.mult(Rho,0,sCompY+i,1);
	    }

	    RhoYdot.resize(box,nSpec+1);
	    cd.reactionRateRhoY(RhoYdot,RhoY,RhoH,T,Patm,box,sCompY,0,sCompT,0);

	    cdfab.setVal(0);
	    for (int i=0; i<cdComps.size(); i++) {
	      for (IntVect iv=box.smallEnd(), be=box.bigEnd(); iv<=be; box.next(iv)) {
		Real val = RhoYdot(iv,cdComps[i]);
		if (val > 0) {
		  cdfab(iv,i) = val / cd.speciesMolecWt()[cdComps[i]]; // Cdot_i = rhoYdot_i/W_i
		}
		else {
		  cdfab(iv,i+cdComps.size()) = val / cd.speciesMolecWt()[cdComps[i]];
		}
	      }
	    }
        }
    ...
    Array<string> outNames(2*cdNames.size());
    for (int i=0; i<cdNames.size(); ++i) {
      outNames[i] = cdNames[i] + "_Creation";
      outNames[i+cdNames.size()] = cdNames[i] + "_Destruction";
    }
