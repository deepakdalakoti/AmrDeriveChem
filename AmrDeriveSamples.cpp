...
//    BoxArray ba = amrData.boxArray(finestLevel);
    ...
    std::string file = "sample.txt"; pp.query("samplefile",file);
    Real samplesize = 0.2 ; pp.query("samplesize",samplesize);
    std::cout << samplesize << std::endl;

    samplefile.open(file.c_str(),  std::ios::out| std::ios_base::app);
   for (MFIter mfi(data_sub[0]); mfi.isValid() ; ++mfi )  {
     int proc = ParallelDescriptor::MyProc();
      ...
      int nz = vbox.length(2);
      int ny = vbox.length(1);
      int nx = vbox.length(0);
     
        for (int k=0 ; k < nz ; k++ ) {
                  
              Real ax_pos = plo[2]+(k+0.5)*dx[2] ;

               for (int j=0 ; j < ny ; j++ ) {
                   for (int i=0 ; i < nx; i++ )  {

                      Real rnd = BoxLib::Random(); 
                      int cell = (k*ny+j)*nx+i;
                       if ( data_sub[0][mfi].dataPtr(indZ)[cell] < 0.008 )  continue;
                         if (rnd < samplesize) { 
                           samplefile << ax_pos  ;
                           for (int com = 0 ; com < comps.size() ; com++ ) {
                             samplefile << " "  ;
                             Real* data = data_sub[0][mfi].dataPtr(com); 
                             samplefile << data[cell]  ;
                                                         }
                             samplefile << "\n" ;
                                      }
