      ...
      char* argv[])
{
   ...
   for(int i=0; i<inVarNames.size(); i++) {
    ...
    int RR_OH = 0;
    for (int i=0 ; i < varNames.size() ; i++ )
             if(varNames[i]=="I_R[rhoY(OH)]") RR_OH = i ;
    for (int lev=0; lev<Nlev; ++lev)
    {
        const BoxArray ba = amrData.boxArray(lev);
        MultiFab inState(ba,nCompIn,nGrow);
        MultiFab Vars(ba,varNames.size(),0);
        outState.set(lev,new MultiFab(ba,(2*BL_SPACEDIM+1+1+1)*si+varNames.size(),0));
...
//           std::cout << "checkpoint" << std::endl;
            ...
            FORT_COMPDIFFTERMS(fluxBox.loVect(),fluxBox.hiVect(),vbox.loVect(),vbox.hiVect(),
                               ...
                               dx.dataPtr(), &nSpec, &si,NwhichSpec.dataPtr(),AdvT.dataPtr(),total.dataPtr(),Y.dataPtr(nCompIn-1), var.dataPtr(),&RR_OH,&tot_in);
     ...
     for (int i=0; i<si; i++)  {
         ...
         outB.copy(total,i,i*8+4,1); //total for now has beeen replace dby Da

//         outB.plus(Reac,NwhichSpec[i],i*8+4,1);
         outB.copy(AdvT,0,(i)*8+5,3);

         outB.copy(Y,NwhichSpec[i],i*8+8,1); 

           }
         outB.copy(var,0,(si)*9,varNames.size());
    ...
    Array<string> nnames((2*BL_SPACEDIM+1+1+1)*whichSpec.size()+varNames.size());
  for (int j=0 ; j<whichSpec.size(); ++j) {
   
#if BL_SPACEDIM==2    
        nnames[2*j*BL_SPACEDIM] = "Adv_Z_" + whichSpec[j];
        nnames[2*j*BL_SPACEDIM+1] = "Adv_T1_" + whichSpec[j];  
        
          nnames[2*j*BL_SPACEDIM+2]= "Diff_Z_" + whichSpec[j] ;
          nnames[2*j*BL_SPACEDIM+3]= "Diff_T1_" + whichSpec[j] ;
#else
        ...
        nnames[2*j*(BL_SPACEDIM+1)+2] = "Diff_T2_"+whichSpec[j]; 
         ...
         nnames[2*j*(BL_SPACEDIM+1)+8] = whichSpec[j];
#endif
    }
       for (int i=0; i<varNames.size(); i++) 
              nnames[si*9+i] = varNames[i];
    bool verb=false;
