      ...
      char* argv[])
{
    ...
    for (int lev=0; lev<Nlev; ++lev)
    {
        ...
        outState.set(lev,new MultiFab(ba,(2*BL_SPACEDIM+1)*si+varNames.size(),0));
        ...
        FArrayBox RhoD,Reac,rhoY,Y,rhoH;
//     for(int k=0 ; k<whichSpec.size(); ++k) {  
        for (MFIter mfi(inState); mfi.isValid(); ++mfi)
        {
            ...
            rhoH.resize(vbox,1);
            Y.resize(vbox,nSpec);
            AdvT.resize(vbox,3*whichSpec.size());
     ...
     for (int i=0; i<si; i++)  {
         outB.copy(DTerms,NwhichSpec[i],(i)*7,1);
         outB.copy(DTerms,NwhichSpec[i]+nSpec,(i)*7+1,1);
         outB.copy(DTerms,NwhichSpec[i]+nSpec+nSpec,(i)*7+2,1);
         outB.copy(Reac,NwhichSpec[i],(i)*7+3,1);
         outB.copy(AdvT,0,(i)*7+4,3); 
           }
         outB.copy(var,0,(si)*7,varNames.size());
    ...
    Array<string> nnames((2*BL_SPACEDIM+1)*whichSpec.size()+varNames.size());
  for (int j=0 ; j<whichSpec.size(); ++j) {
   
#if BL_SPACEDIM==2    
        nnames[2*j*BL_SPACEDIM] = "Adv_X_" + whichSpec[j];
        nnames[2*j*BL_SPACEDIM+1] = "Adv_Y_" + whichSpec[j];  
        
          nnames[2*j*BL_SPACEDIM+2]= "Diff_X_" + whichSpec[j] ;
          nnames[2*j*BL_SPACEDIM+3]= "Diff_Y_" + whichSpec[j] ;
#else
         nnames[2*j*BL_SPACEDIM] = "Diff_X_" + whichSpec[j];
        nnames[2*j*BL_SPACEDIM+1] = "Diff_Y_" + whichSpec[j];  
        nnames[2*j*BL_SPACEDIM+2] = "Diff_Z_"+whichSpec[j]; 
          nnames[2*j*BL_SPACEDIM+3]= "Reac_" + whichSpec[j] ;
          nnames[2*j*BL_SPACEDIM+4]= "Adv_X_" + whichSpec[j] ;
         nnames[2*j*BL_SPACEDIM+5] = "Adv_Y_" + whichSpec[j];
         nnames[2*j*BL_SPACEDIM+6] = "Adv_Z_";
#endif
    }
       for (int i=0; i<varNames.size(); i++) 
              nnames[si*7+i] = varNames[i];
    bool verb=false;
