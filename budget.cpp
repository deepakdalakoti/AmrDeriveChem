...
//           std::cout << "checkpoint" << std::endl;
  ...
  for (int j=0 ; j<whichSpec.size(); ++j) {
        ...
        nnames[2*j*(BL_SPACEDIM+1)+2] = "Diff_Z_"+whichSpec[j]; 
          nnames[2*j*(BL_SPACEDIM+1)+3]= "Reac_" + whichSpec[j] ;
          nnames[2*j*(BL_SPACEDIM+1)+4] = "total_" + whichSpec[j];
          nnames[2*j*(BL_SPACEDIM+1)+5]= "Adv_X_" + whichSpec[j] ;
         nnames[2*j*(BL_SPACEDIM+1)+6] = "Adv_Y_" + whichSpec[j];
       
         nnames[2*j*(BL_SPACEDIM+1)+7] = "Adv_Z_" + whichSpec[j];
#endif
    }
       for (int i=0; i<varNames.size(); i++) 
