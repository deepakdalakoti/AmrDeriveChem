      ...
      char* argv[])
{
    ...
    std::string whichSpec;
    pp.query("whichSpec",whichSpec);
    int ind = cd.index(whichSpec);
 //   std::cout << "which Spec "<< whichSpec[0] << whichSpec.size() << std::endl;
    ...
    inVarNames[nSpec] = "temp";
    Array<std::string> invar2(1);
    invar2[0] = "density";
    Array<int> dest2(1);
    dest2[0] = 0;
    PArray<MultiFab> outState(Nlev,PArrayManage);
    PArray<Geometry> geoms(Nlev,PArrayManage);
   if(ParallelDescriptor::IOProcessor()) std::cout << inVarNames[ind] << std::endl; 
    const int nGrow = 1;
    ...
    for (int lev=0; lev<Nlev; ++lev)
    {
        ...
        MultiFab rho(ba,1,0);
        outState.set(lev,new MultiFab(ba,1,0));
        ...
        amrData.FillVar(rho,lev,invar2,dest2);
        ...
        FArrayBox RhoD,Reac,rhoY,rhoH,H;
     for(int k=0 ; k<whichSpec.size(); ++k) {  
        for (MFIter mfi(inState); mfi.isValid(); ++mfi)
        {
           ...
           H.resize(vbox,1);
            ...
            cd.getHmixGivenTY(H,T,Y,vbox,nSpec,0,0);
            H.mult(rho[mfi],0,0,1);
            for (int spe=0 ; spe < nSpec; spe++) {
                     
            rhoY.mult(rho[mfi],0,spe,1);
          ...
          // Compute some stuff on grown box
            ...
            FORT_MIXAVG_RHODIFF_TEMP(gbox.loVect(),gbox.hiVect(),       // Pass rhoY for SDC version
                                     RhoD.dataPtr(),ARLIM(RhoD.loVect()),ARLIM(RhoD.hiVect()),
                                     T.dataPtr(nSpec),ARLIM(T.loVect()),ARLIM(T.hiVect()),
            ...
            FORT_COMPDIFFTERMS(vbox.loVect(),vbox.hiVect(),
                               ...
                               dx.dataPtr(), &nSpec, &ind,Reac.dataPtr(ind));
  ...
  std::string outfile(getFileRoot(plotFileName) + "_DamKoh_"+whichSpec); pp.query("outfile",outfile);
  
  if (ParallelDescriptor::IOProcessor())
      cout << "Writing new data to " << outfile << endl;
  
  Array<std::string> nnames(1);
  nnames[0] = "Damkoh_" + whichSpec;
 ...
 bool verb=false;
  const AmrData& a = amrData;
   ...
   if(append)
   appendToPlotFile(amrData, outState, plotFileName, nnames,"Damkoh",0);
   else
