...
//           if(nC >0 && nH >0) {
//            if(cd.speciesNames()[k]=="CH2O"|| cd.speciesNames()[k]=="NC12H26"|| cd.speciesNames()[k]=="H2O2"|| cd.speciesNames()[k]=="C12H25O2") {
           if( cd.speciesNames()[k]=="NC12H26") {
            if(ParallelDescriptor::IOProcessor())
...
//        geoms[lev].FillPeriodicBoundary(Specs[lev],0,nCompIn,do_corners);
        ...
        for (MFIter mfi(Specs[lev]); mfi.isValid(); ++mfi)
        {
            ...
            FORT_GRAD(box.loVect(),box.hiVect(),
                      ...
                      ARLIM(OutBox.hiVect()),s.dataPtr(nSpec));
        }
...
//        Array<std::string> nnames(1);
       ...
       appendToPlotFile(amrData, Out, plotFileName, nnames,"FlameIndex",0);
       }
