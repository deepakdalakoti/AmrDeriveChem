      ...
      char* argv[])
{
   ...
   for(int i=0; i<inVarNames.size(); i++) {
    ...
    for (int lev=0; lev<Nlev; ++lev)
    {
        ...
        outState.set(lev,new MultiFab(ba,(2*BL_SPACEDIM+1+1)*si+varNames.size()+4,0));
...
//        geoms[lev].FillPeriodicBoundary(inState,0,nCompIn,do_corners);
        ...
        FArrayBox RhoD,Reac,rhoY,rhoH,total, cp;
...
//           std::cout << "checkpoint" << std::endl;
            ...
            FORT_COMPDIFFTERMS(fluxBox.loVect(),fluxBox.hiVect(),vbox.loVect(),vbox.hiVect(),
                               ...
                               dx.dataPtr(), &nSpec, &si,NwhichSpec.dataPtr(),AdvT.dataPtr(),total.dataPtr(),cp.dataPtr(), T.dataPtr(idTst), DiffT.dataPtr());
...
#if BL_SPACEDIM==2
         outB.copy(DTerms,NwhichSpec[i],(i)*6,1);
         outB.copy(DTerms,NwhichSpec[i]+nSpec,(i)*6+1,1);
         outB.copy(Reac,NwhichSpec[i],(i)*6+2,1);
         outB.copy(total,i,i*6+3,1);
//         outB.plus(Reac,NwhichSpec[i],i*8+3,1); better estimates of reactions from I_R
         outB.copy(AdvT,i*BL_SPACEDIM,(i)*6+4,2);
#endif
      ...
      outB.copy(DiffT,0,si*6,4);
      outB.copy(var,0,(si)*6+4,varNames.size());
...
// Write a line to a text file 
    ...
    Array<string> nnames((2*BL_SPACEDIM+1+1)*whichSpec.size()+varNames.size()+4);
  for (int j=0 ; j<whichSpec.size(); ++j) {
   
#if BL_SPACEDIM==2    
        nnames[2*j*(BL_SPACEDIM+1)] = "Diff_X_" + whichSpec[j];
        nnames[2*j*(BL_SPACEDIM+1)+1] = "Diff_Y_" + whichSpec[j];  
        nnames[2*j*(BL_SPACEDIM+1)+2] = "Reac_" + whichSpec[j];  
        nnames[2*j*(BL_SPACEDIM+1)+3] = "total_" + whichSpec[j];  


        nnames[2*j*(BL_SPACEDIM+1)+4]= "Adv_X_" + whichSpec[j] ;
        nnames[2*j*(BL_SPACEDIM+1)+5]= "Adv_Y_" + whichSpec[j] ;
...
#if BL_SPACEDIM ==2
       nnames[si*6]= "DiffT_X" ;
        nnames[si*6+1]= "DiffT_Y"  ;
        nnames[si*6+2]= "AdvT_X"  ;

        nnames[si*6+3]= "AdvT_Y"  ;
#endif
