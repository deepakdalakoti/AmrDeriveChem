      ...
      char* argv[])
{
 ...
 DataServices::SetBatchMode();
  ...
  beta[nC] = 2/elementAtomicWt[nC];
  beta[nH] = 1/(2*elementAtomicWt[nH]);
  beta[nO] = -1/elementAtomicWt[nO];
  ...
  for (int spec = 0; spec<nSpec; ++spec) {
    for (int elt = 0; elt<nElts; ++elt) {
      mu[spec][elt] = Real(cd.numberOfElementXinSpeciesY(elementNames[elt],speciesNames[spec]))
        * Real(elementAtomicWt[elt]) / Real(speciesMolecWt[spec]);
    }
  }
...
//        std::cout << "mu_FU["<< speciesNames[spec] << "][" << elementNames[elt]<< "] = " << mu[spec][elt] << std::endl;
  ...
  WritePlotFile(mf,outfile,outNames,amrData);
  BoxLib::Finalize();
  return 0;
}
